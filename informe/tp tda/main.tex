...
def calcular_fuerza(q, C=1):
    ...
    t2 = time.time()
    print(F\    print(\"Tiempo de ejecucion:", t2 - t1)
    return F
	\end{lstlisting}
\end{itemize}
...
\begin{itemize}
	\item Se supone que las distancias se encuentran en una lista, donde cada posición es la distancia a la anterior posición, y cada posición representa una parada. El resultado se genera almacenando los subíndices de cada parada.
        \item Se supone que los valores de los enteros almacenados en la lista no superan 7.
\end{itemize}
...
\begin{itemize}
    ...
    Se utiliza la estructura de datos "Lista simple"

\subsection{Pseudocodigo}
	\begin{lstlisting}[frame=single]
		 
        funcion calcular_mejor_distancia(distancias):
            Iniciar t1 como el tiempo actual
            Inicializar i como 0
            Obtener largo como la longitud de distancias
            Crear lista_con_resultados como una lista vacia
        
            Mientras i sea menor que largo:
                Inicializar auxiliar como 0
                Mientras i sea menor que largo y auxiliar + distancias[i] sea menor o igual a MAXIMA_DISTANCIA:
                    Sumar distancias[i] a auxiliar
                    Incrementar i en 1
                Agregar i-1 a lista_con_resultados
        
            Agregar i a lista_con_resultados
        
            Iniciar t2 como el tiempo actual
            Imprimir lista_con_resultados
            Imprimir "Tiempo de ejecucion:" seguido de t2 - t1
        
            Retornar lista_con_resultados
        ]
	\end{lstlisting}
...
\subsection{Generacion de sets}
\begin{itemize}
\item para generar los sets se realiza de la siguiente manera, utilizando 1 como semilla para el largo 1000, 2 como semilla para el largo 100000, 3, como semilla para largo 100000, 4 como semilla para largo 500000, y 5 como semilla para largo 1000000. Los sets de largo 50, 5000, 20000 y 30000, estan generados con la semilla 21. 
