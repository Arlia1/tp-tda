...
\begin{itemize}
	\item  n es un entero positivo al cual queremos calcular la suma encadenada de longitud minima
    \item  una llamada recursiva puede generar una cantidad de llamadas recursivas igual a len(recorrido)
	\item Por cada nivel del arbol de deciciones la cantidad de llamadas recursivas es igual k! siendo k = el nivel
	\item las cantidad de llamdas recursivas no depende totalmente de n si no mas bien en que nivel del arbol encontramos la primer suma encadena cumpla los requisitos
\end{itemize}
    Por lo tanto si k lo definimos como el nivel en el cual se encuantra la primer suma encadenada que cumple que es n, entonces la complejidad seria O(k!) en el peor de los casos porque por cada nivel se generan k! llamadas recursivas
